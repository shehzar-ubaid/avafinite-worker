...
# import comfy_aimdo.model_vbar  <-- Disabled for AMD
    ...
    try:
        if unique_id in pending_async_nodes:
            ...
            try:
                ...
                # comfy_aimdo.model_vbar.vbars_reset_watermark_limits() <-- Disabled for AMD
        ...
        if input_data_all is not None:
            input_data_formatted = {name: [format_value(x) for x in inputs] for name, inputs in input_data_all.items()}
    ...
    for x in valid_inputs:
        ...
        if isinstance(val, list):
            ...
            try:
                ...
                if input_type == "INT": inputs[x] = int(val)
                elif input_type == "FLOAT": inputs[x] = float(val)
                elif input_type == "STRING": inputs[x] = str(val)
                elif input_type == "BOOLEAN": inputs[x] = bool(val)
            except Exception as ex:
                error = {
            ...
            if x not in validate_function_inputs and not validate_has_kwargs:
                if "min" in extra_info and val < extra_info["min"]:
                    errors.append({"type": "value_smaller_than_min", "message": f"Value {val} smaller than min of {extra_info['min']}", "details": f"{x}", "extra_info": {"input_name": x, "input_config": info, "received_value": val}})
                    continue
                if "max" in extra_info and val > extra_info["max"]:
                    errors.append({"type": "value_bigger_than_max", "message": f"Value {val} bigger than max of {extra_info['max']}", "details": f"{x}", "extra_info": {"input_name": x, "input_config": info, "received_value": val}})
                    continue
                if isinstance(input_type, list) or input_type == io.Combo.io_type:
                    ...
                    if val not in combo_options:
                        list_info = f"(list of length {len(combo_options)})" if len(combo_options) > 20 else str(combo_options)
                        errors.append({"type": "value_not_in_list", "message": "Value not in list", "details": f"{x}: '{val}' not in {list_info}", "extra_info": {"input_name": x, "input_config": info if len(combo_options) <= 20 else None, "received_value": val}})
                        continue

    if len(validate_function_inputs) > 0 or validate_has_kwargs:
        input_data_all, _, v3_data = get_input_data(inputs, obj_class, unique_id)
        input_filtered = {x: input_data_all[x] for x in input_data_all if x in validate_function_inputs or validate_has_kwargs}
        ...
        for x in input_filtered:
            for i, r in enumerate(ret):
                if r is not True and not isinstance(r, ExecutionBlocker):
                    details = f"{x}" + (f" - {str(r)}" if r is not False else "")
                    errors.append({"type": "custom_validation_failed", "message": "Custom validation failed for node", "details": details, "extra_info": {"input_name": x}})

    ret = (len(errors) == 0 and valid is True, errors, unique_id)
    validated[unique_id] = ret
    return ret

def full_type_name(klass):
    module = klass.__module__
    return klass.__qualname__ if module == 'builtins' else module + '.' + klass.__qualname__

async def validate_prompt(prompt_id, prompt, partial_execution_list: Union[list[str], None]):
    outputs = set()
    for x in prompt:
        if 'class_type' not in prompt[x]:
            node_title = prompt[x].get('_meta', {}).get('title')
            return (False, {"type": "missing_node_type", "message": f"Node '{node_title or f'ID #{x}'}' has no class_type.", "details": f"Node ID '#{x}'", "extra_info": {"node_id": x, "class_type": None, "node_title": node_title}}, [], {})
        class_type = prompt[x]['class_type']
        class_ = nodes.NODE_CLASS_MAPPINGS.get(class_type, None)
        if class_ is None:
            node_title = prompt[x].get('_meta', {}).get('title', class_type)
            return (False, {"type": "missing_node_type", "message": f"Node '{node_title}' not found.", "details": f"Node ID '#{x}'", "extra_info": {"node_id": x, "class_type": class_type, "node_title": node_title}}, [], {})
        if hasattr(class_, 'OUTPUT_NODE') and class_.OUTPUT_NODE is True:
            if partial_execution_list is None or x in partial_execution_list: outputs.add(x)

    if len(outputs) == 0:
        return (False, {"type": "prompt_no_outputs", "message": "Prompt has no outputs", "details": "", "extra_info": {}}, [], {})

    good_outputs, errors, node_errors, validated = set(), [], {}, {}
    for o in outputs:
        try:
            m = await validate_inputs(prompt_id, prompt, o, validated)
            valid, reasons = m[0], m[1]
        except Exception as ex:
            typ, _, tb = sys.exc_info()
            valid, reasons = False, [{"type": "exception_during_validation", "message": "Exception when validating node", "details": str(ex), "extra_info": {"exception_type": full_type_name(typ), "traceback": traceback.format_tb(tb)}}]
            validated[o] = (False, reasons, o)

        if valid: good_outputs.add(o)
        else:
            errors += [(o, reasons)]
            for node_id, result in validated.items():
                if result[0] is not True and len(result[1]) > 0:
                    if node_id not in node_errors:
                        node_errors[node_id] = {"errors": result[1], "dependent_outputs": [], "class_type": prompt[node_id]['class_type']}
                    node_errors[node_id]["dependent_outputs"].append(o)

    if len(good_outputs) == 0:
        errors_list = "\n".join([f"{e['message']}: {e['details']}" for _, errs in errors for e in errs])
        return (False, {"type": "prompt_outputs_failed_validation", "message": "Prompt outputs failed validation", "details": errors_list, "extra_info": {}}, list(good_outputs), node_errors)
...
class PromptQueue:
    def __init__(self, server):
        self.server, self.mutex = server, threading.RLock()
        self.not_empty, self.task_counter = threading.Condition(self.mutex), 0
        self.queue, self.currently_running, self.history, self.flags = [], {}, {}, {}
    ...
    def get(self, timeout=None):
        with self.not_empty:
            while len(self.queue) == 0:
                ...
                if timeout is not None and len(self.queue) == 0: return None
    ...
    def task_done(self, item_id, history_result, status: Optional['PromptQueue.ExecutionStatus'], process_item=None):
        with self.mutex:
            ...
            if len(self.history) > MAXIMUM_HISTORY_SIZE: self.history.pop(next(iter(self.history)))
            status_dict = copy.deepcopy(status._asdict()) if status is not None else None
            if process_item is not None: prompt = process_item(prompt)
            self.history[prompt[1]] = {"prompt": prompt, "outputs": {}, 'status': status_dict}
            self.history[prompt[1]].update(history_result)
            self.server.queue_updated()

    def get_current_queue(self):
        with self.mutex: return ([x for x in self.currently_running.values()], copy.deepcopy(self.queue))

    def get_current_queue_volatile(self):
        with self.mutex: return ([x for x in self.currently_running.values()], copy.copy(self.queue))

    def get_tasks_remaining(self):
        with self.mutex: return len(self.queue) + len(self.currently_running)
        ...
        with self.mutex:
            for x in range(len(self.queue)):
                if function(self.queue[x]):
                    if len(self.queue) == 1: self.wipe_queue()
                    else:
                        self.queue.pop(x)
    ...
    def get_history(self, prompt_id=None, max_items=None, offset=-1, map_function=None):
        with self.mutex:
            if prompt_id is None:
                out, i = {}, 0
                if offset < 0 and max_items is not None: offset = len(self.history) - max_items
                for k in self.history:
                    if i >= offset:
                        p = map_function(self.history[k]) if map_function else self.history[k]
                        out[k] = p
                        if max_items is not None and len(out) >= max_items: break
                    i += 1
                return out
            elif prompt_id in self.history:
                p = map_function(self.history[prompt_id]) if map_function else copy.deepcopy(self.history[prompt_id])
                return {prompt_id: p}
            return {}

    def wipe_history(self):
        with self.mutex: self.history = {}
